...
Two backends, selected by ``SUMMARIZER`` exactly like ``EMBEDDING_MODEL``:
...
def summarize_claude(briefs: Sequence[dict[str, Any]], language: str, *, effort: str = "medium") -> list[TopicSummary]:
    ...
    One call rather than one per topic: it is cheaper (the system prompt is
    cached once), and the model can see that two items are the same customer and
    write them as a set instead of repeating itself.
    """
    import anthropic
    ...
    request: dict[str, Any] = {
        "model": model_name(),
        "max_tokens": 8000,
        # The system prompt is byte-stable across runs, so it caches; the items
        # change every time and are placed after it.
...
def summarize_topics(
    ...
    if chosen == "template":
        ...
        raise SystemExit(f"Unknown SUMMARIZER {chosen!r}. Use 'template' or 'claude'.")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--backend", choices=("template", "claude"), help="Overrides SUMMARIZER for this run")
