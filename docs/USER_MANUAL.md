...
ถ้าขึ้น help และ pytest ผ่านหมด (**178 passed**) ถือว่าฝั่ง Python พร้อม
...
| `TAM_NAMES` | ไม่ | ว่างไว้ = ใช้ชื่อจริงถ้ามี cache · `pseudonym` เวลาเดโม/แคปหน้าจอ · ดึงชื่อครั้งเดียวด้วย `python3 -m tam.ingest.users --fetch` · **ตั้งให้ตรงกับ `slack-bot/.env`** เพราะ bot แปลชื่อเองจากไฟล์เดียวกัน (`TAM_NAMES_PATH`, default `../pipeline/data/user_names.json`) |
...
ก่อนเสิร์ฟ มันจะพิมพ์ `Ready: 29 record(s), 11 topic(s), 4 blocked` แล้วเปิด
<http://localhost:8899> ได้เลย — หน้า digest มีงาน 11 ชิ้น ติดอยู่ 4 ชิ้น กดเข้าไปดู
timeline ของแต่ละชิ้นได้

> **ข้อจำกัดของไฟล์ตัวอย่าง** บนคลังเล็กขนาดนี้ Slack กับที่ประชุม**ยังไม่รวมเข้างานชิ้นเดียวกัน**
> (แต่ละงานมีที่มาเดียว) การรวมข้ามที่มาเห็นได้บนคลังจริงที่มีบทสนทนาต่อเนื่องพอ —
> ตัวอย่างจึงพิสูจน์ว่า pipeline เดินครบทุกขั้น ไม่ได้พิสูจน์ข้อนี้
...
### 5.3 แชทที่ export ไม่ถึง — copy มาวาง

token ดึงได้เฉพาะห้องที่เชิญบอทเข้าไป แต่เรื่องที่ตัดสินกันจริงมักอยู่ใน DM กลุ่มส่วนตัว
หรือ workspace ของลูกค้า วิธีเอาเข้ามาคือ **ลากเลือกข้อความใน Slack → copy → วาง**

ทางหน้าจอ: `/upload` → ช่อง "หรือ copy แชทจาก Slack มาวาง" → ใส่ว่าคุยที่ไหน + วันแรกของ
บทสนทนา → กด **ดูก่อนว่าอ่านถูกไหม** ระบบจะแสดงว่าอ่านได้กี่ข้อความ ใครพูดอะไรเมื่อไหร่
และอะไรที่อ่านไม่ออก **ยังไม่เขียนอะไรลง corpus จนกว่าจะกดยืนยัน**

ทาง CLI:

```bash
pbpaste | python3 -m tam.ingest.slack_paste --title "DM พี่ Natta" --date 2026-08-19 --dry-run
pbpaste | python3 -m tam.ingest.slack_paste --title "DM พี่ Natta" --date 2026-08-19 \
        --merge-into data/processed/messages.json
```

สิ่งที่ระบบทำให้เอง และเหตุผล:

| สิ่งที่ทำ | ทำไม |
|---|---|
| อ่านรูปแบบ `ชื่อ  [2:21 PM]` + บรรทัดเวลาเปล่า `[2:22 PM]…` เป็นข้อความถัดไปของคนเดิม | นี่คือสิ่งที่ Slack copy ออกมาจริง ๆ |
| ตัด `6 replies`, ชื่อไฟล์รูปที่ติดหน้าชื่อคน, `(edited)`, แถวรีแอ็กชัน | ของแถมพวกนี้เชื่อมติดกับชื่อคนพูดโดยไม่มีตัวคั่น |
| วงเล็บเหลี่ยมที่**ไม่ใช่เวลา** เช่น `[BUG][View redemption result]` อยู่ในเนื้อความตามเดิม | ไม่งั้นประโยคเดียวจะถูกหั่นเป็นสามข้อความ และเกิดคนพูดปลอมชื่อ `เรื่องคอลัม REVERAPP-228` |
| ข้อความที่คนเดียวกันพิมพ์ติดกันภายใน 2 นาที = **หนึ่ง record** ต่อกันด้วยการขึ้นบรรทัดใหม่ | `ของผมสุดท้ายคือแตก` แล้วต่อ `400` คือประโยคเดียว · ขึ้นบรรทัดใหม่ (ไม่ใช่เว้นวรรค) เพื่อให้ตัวอ่าน blocker ระดับบรรทัดยังอ้างบรรทัดนั้นเป็นหลักฐานได้ |
| **ไม่ใส่ `thread_ts`** ให้ทั้งก้อน แต่ให้ `channel_id` ของตัวเอง (`paste-…`) | สกรอลล์ที่ copy มาไม่ใช่เธรด — `6 replies` เป็นหลักฐานว่า reply ไม่ได้ถูก copy มาด้วย · ถ้าใส่ thread เดียวกันทั้งก้อน graph จะเชื่อว่าทุกข้อความเป็นเรื่องเดียวกัน |
| ชื่อที่ตรงกับ cache ชื่อ (`tam.ingest.users --fetch`) ถูกแปลงกลับเป็น Slack id | ไม่งั้นคนคนเดียวจะกลายเป็นสองแถวในหน้า `/people` — แถวหนึ่งเป็น id อีกแถวเป็นชื่อ · ชื่อที่ซ้ำกันสองคนจะไม่เดา เก็บเป็นชื่อตามที่วาง |
| id เป็น hash ของ (ห้อง, คนพูด, เวลา, ข้อความ) | วางซ้ำทับกันได้ — ข้อความเดิมถูก**แทนที่** ไม่ใช่เพิ่มซ้ำ ไม่ต้องจำว่าวางถึงไหนแล้ว |

ข้อจำกัดที่ต้องรู้: คลิปบอร์ดมีแต่**เวลา ไม่มีวันที่** จึงต้องบอกวันเอง (ถ้าเวลาเดินถอยหลัง
ระบบถือว่าข้ามเที่ยงคืน) · ข้อความที่ copy มาไม่มี permalink กลับไปหาต้นฉบับ เพราะไม่มี id
ของ Slack ติดมาด้วย · บนหน้าจอ ใน Slack card และในคอมเมนต์ที่เขียนลง ticket ที่มาจะขึ้นว่า
**Slack (แชทปิด)** (📋) ไม่ใช่ **Slack** เพราะข้อความที่ export มาคือข้อความเต็ม
ส่วนข้อความที่วางคือเท่าที่คนเลือกมา — และอันนี้ไม่มี permalink ให้กดตรวจ
...
INFO Ready: 29 record(s), 11 topic(s), 4 blocked, summariser template
```
...
| `/people` | ใครอยู่กับเรื่องไหน — 25 คนบนคลังที่รันอยู่ พร้อมจำนวนงานที่เกี่ยวข้อง งานที่ติด และข้อความที่พูด · กดชื่อคนเพื่อเปิดหน้าของคนนั้น |
...
| `/upload` | สามทางเข้าในหน้าเดียว — **วางโน้ตที่จดเอง**, **วางแชทที่ copy จาก Slack**, หรืออัปโหลดไฟล์ถอดเสียง · แชทที่วางจะ**ให้ดูก่อนว่าอ่านถูกไหม** แล้วค่อยกดยืนยัน (ดูข้อ 5.3) · **วางโน้ตที่จดเอง** (ที่ใช้จริงบ่อยสุด — ทีมส่วนใหญ่ไม่มีไฟล์ถอดเสียง) หรืออัปโหลด `.vtt` / `.srt` · วางหนึ่งครั้ง = หนึ่ง record · วางซ้ำในวันเดียวกันแทนที่ของเดิม · บรรทัดแบบ `• Pending …` จะขึ้นใน `/blockers` โดยอ้างบรรทัดนั้นเป็นหลักฐาน · CLI: `python3 -m tam.ingest.notes` |
...
| `/meowtam @someone` | คนนั้นกำลังทำอะไร — ใส่ชื่อที่เห็นบนหน้าจอก็ได้ ไม่ต้องใส่ `U0…` |
...
| บันทึกเป็นการตัดสินใจ | เก็บเข้า decision log · ปัก 🧠 ไว้ที่ข้อความให้ทั้งห้องเห็น · ขึ้นบนการ์ดของงานที่ข้อความนั้นสังกัด · หาเจอด้วย `recall` ได้ตลอด |
...
npm test                       # ชุดทดสอบฝั่งบอท — 136 ผ่าน (รวม tests/gate.test.ts)
...
กับ export จริง 936 record ที่เสิร์ฟด้วย `BAAI/bge-m3` (คลังวันนี้โตเป็น 1,301 record แล้ว
และ `check-api` วัดใหม่ทุกครั้งที่รัน — ผลล่าสุดยังเป็น 3/4 เท่าเดิม):
...
# copy แชทจาก Slack แล้ววาง (DM / กลุ่มส่วนตัว ที่ export ไม่ถึง) — --dry-run คือดูก่อน ไม่เขียนอะไร
pbpaste | python3 -m tam.ingest.slack_paste --title "DM พี่ Natta" --date 2026-08-19 --dry-run
