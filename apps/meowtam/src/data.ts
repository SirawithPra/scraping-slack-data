...
export function ledger(): Ledger {
  if (!cache) cache = JSON.parse(readFileSync(LEDGER_PATH, 'utf8')) as Ledger;
  return cache;
}

/** Re-read from disk. Used after `npm run ledger` so you needn't restart mid-demo. */
export function reload(): Ledger {
  cache = null;
  return ledger();
}
